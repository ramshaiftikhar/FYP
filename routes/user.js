...
router.post("/", (req, res) => {
  ...
  // from req.body - which will be { email: 'rohanpaul2@gmail.com', password: '123456' }
  // assign the email and password variable with ES-6-destructuring to be equal the value of req.body.email and req.body.password respectively
  ...
  User.findOne({ email: email }, (err, user) => {
    if (err) {
      ...
      res.json({
        error: `Sorry, that email already exists with ${email}`
      });
    } else {
